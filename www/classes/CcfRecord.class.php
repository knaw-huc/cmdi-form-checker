...
class Ccfrecord {
        ...
        foreach ($headerItems->childNodes as $item) {
            switch ($item->tagName) {
                ...
                case "cmd:MdProfile":
                    ...
                    break;
                case "cmd:MdCreator":
                    //temp fix voor invoerder
                    if (isset($inputComponents[0]["content"][0]["content"][0]["value"])) {
                        if ($inputComponents[0]["content"][0]["name"] == "creator") {
                            $item->nodeValue = $inputComponents[0]["content"][0]["content"][0]["value"];
                        }
                    }
                    break;
            }
    ...
    private function processElements($name, $items, $node, $cmdi) {
        foreach ($items as $item) {
            $newNode = $cmdi->createElement('cmd:' . $name, str_replace("\xEF\xBB\xBF",'',htmlspecialchars($item["value"])));
